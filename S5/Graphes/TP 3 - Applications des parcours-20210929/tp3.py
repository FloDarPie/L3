    ...
    for sommet in G:
        if not sommet in Visite:
            ...
            while len(File) > 0:
                pereCourant = File[0]
                File = File[1:]
                for successeur in G[pereCourant]:
                    if not successeur in Visite:
                        Visite[successeur] = 3 - Visite[pereCourant]
                        if Visite[successeur] == 1:
                            Red.add( successeur )
                        else:
                            Blu.add( successeur )
                        File.append(successeur)
                    elif Visite[successeur] == Visite[pereCourant]:
                        return False, {}, {}
    return True, Red, Blu


biparti({1:[2,3,7],2:[1,5,6],3:[1,5],4:[6,8],5:[2,3,7],6:[2,4],7:[1,5,8],8:[4,7]})
